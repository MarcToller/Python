...
if cpf_valido:
    for posicao_digito in [10,11]:
        if len(cpf_parcial) == 0:
            cpf_parcial = cpf_somente_numeros[:posicao_digito-1]            

        soma_digitos = 0

        for indice_calculo, digito in enumerate(cpf_parcial, start=posicao_digito*-1):
            soma_digitos += (int(digito) * (indice_calculo*-1)) * 10

        resto = soma_digitos % 11    

        cpf_digito_calculado = resto if resto <= 9 else 0
        cpf_parcial = cpf_parcial + str(cpf_digito_calculado)     
            
cpf_valido = cpf_parcial == cpf_somente_numeros

if cpf_valido:
    print('CPF válido')                  
else:      
    print('CPF inválido: CPF Digitado: ', cpf_somente_numeros, 'CPF Correto:', cpf_parcial)